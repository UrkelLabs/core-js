...
class JsonRpcServer {
  ...
  constructor(config, minerConfig, poolConfig) {
    ...
    if (typeof config.corsdomain === "string")
      config.corsdomain = [config.corsdomain];
    ...
    http
      .createServer((req, res) => {
        ...
        if (!Nimiq.NetUtils.isLocalIP(remoteIp)) {
          // Not local host
          let found = false;
  ...
  async init(client, consensus, miner, walletStore) {
    ...
    this._methods.set(
      ...
      this.getTransactionsByAddress.bind(this)
    );
    ...
    this._methods.set(
      "poolConnectionState",
      this.poolConnectionState.bind(this)
    );
    this._methods.set(
      "poolConfirmedBalance",
      this.poolConfirmedBalance.bind(this)
    );
  ...
  static _authenticate(req, res, username, password) {
    if (
      ...
      res.writeHead(401, {
        "WWW-Authenticate":
          'Basic realm="Use user-defined username and password to access the JSON-RPC API." charset="UTF-8"',
      });
      res.end();
      return false;
    }
  ...
  async syncing() {
    if (this._consensusState === Nimiq.Client.ConsensusState.ESTABLISHED)
      return false;
  ...
  async peerList() {
    ...
    for (const address of addresses) {
      peers.push(
        this._infoToPeerObj(
          allPeers.find((peer) => peer.peerAddress.equals(address.peerAddress)),
          address
        )
      );
    }
  ...
  async peerState(peer, set) {
    ...
    return this._infoToPeerObj(
      await this._client.network.getPeer(peer),
      await this._client.network.getAddress(peer)
    );
  }
  ...
  async createRawTransaction(tx) {
    ...
    const fromType = tx.fromType
      ? Number.parseInt(tx.fromType)
      : Nimiq.Account.Type.BASIC;
    const to =
      !tx.to && flags == Nimiq.Transaction.Flag.CONTRACT_CREATION
        ? Nimiq.Address.CONTRACT_CREATION
        : Nimiq.Address.fromString(tx.to);
    const toType = tx.toType
      ? Number.parseInt(tx.toType)
      : Nimiq.Account.Type.BASIC;
    const value = parseInt(tx.value);
    const fee = parseInt(tx.fee);
    const data = tx.data
      ? Nimiq.BufferUtils.fromHex(tx.data)
      : new Uint8Array(0);
    const validityStartHeight = tx.validityStartHeight
      ? Number.parseInt(tx.validityStartHeight)
      : await this._client.getHeadHeight();
    /** @type {Wallet} */
    const wallet = await this._walletStore.get(signer);
    if (!wallet || !(wallet instanceof Nimiq.Wallet)) {
      throw new Error(
        `"${signer.toUserFriendlyAddress()}" can not sign transactions using this node.`
      );
    }
    let transaction;
    if (
      fromType !== Nimiq.Account.Type.BASIC ||
      toType !== Nimiq.Account.Type.BASIC ||
      data.length != 0
    ) {
      let proof;
      if (fromType === Nimiq.Account.Type.HTLC) {
        if (!tx.proof)
          throw new Error(
            "For transactions from HTLCs, proof data is required."
          );
        switch (tx.proof.type) {
          case Nimiq.HashedTimeLockedContract.ProofType.REGULAR_TRANSFER: {
            ...
            const hashRoot = Nimiq.Hash.fromAny(
              tx.proof.hashRoot,
              hashAlgorithm
            );
            const preImage = Nimiq.Hash.fromAny(
              tx.proof.preImage,
              hashAlgorithm
            );

            proof = new Nimiq.SerialBuffer(
              1 +
                1 +
                1 +
                2 * Nimiq.Hash.SIZE.get(hashAlgorithm) +
                Nimiq.SignatureProof.SINGLE_SIG_SIZE
            );
          ...
          case Nimiq.HashedTimeLockedContract.ProofType.TIMEOUT_RESOLVE: {
            proof = new Nimiq.SerialBuffer(
              1 + Nimiq.SignatureProof.SINGLE_SIG_SIZE
            );
            proof.writeUint8(tx.proof.type);
            break;
          }
  ...
  async getTransactionByBlockHashAndIndex(blockHash, txIndex) {
    const block = await this._client.getBlock(
      Nimiq.Hash.fromString(blockHash),
      true
    );
    if (block && block.transactions.length > txIndex) {
      return this._transactionToObj(
  ...
  async getTransactionReceipt(hash) {
    ...
    return {
      ...
      transactionIndex: block
        ? block.transactions.findIndex((tx) => tx.hash().toHex() === hash)
        : undefined,
  ...
  async getTransactionsByAddress(addr, limit = 1000) {
    ...
    const txs = await this._client.getTransactionsByAddress(
      address,
      0,
      [],
      limit
    ...
    for (const tx of txs) {
      try {
        ...
        Nimiq.Log.w(
          JsonRpcServer,
          `Transactions from receipt is not available`
        );
      }
  ...
  async mempoolContent(includeTransactions) {
    return Promise.all(
      (await this._client.mempool.getTransactions()).map(async (txHash) => {
        return includeTransactions
          ? this._transactionDetailsToObj(
              await this._client.getTransaction(txHash)
            )
          : txHash.toHex();
      })
    );
  ...
  mining(enabled) {
    if (enabled === true) {
      ...
      if (
        !this._miner.working &&
        this._consensusState === Nimiq.Client.ConsensusState.ESTABLISHED
      )
      ...
      if (
        this._miner instanceof Nimiq.BasePoolMiner &&
        !this._miner.isDisconnected()
      ) {
  ...
  pool(pool) {
    ...
    return this._poolConfig.enabled &&
      this._isPoolValid(this._miner.host, this._miner.port)
  ...
  async getWork(addressStr, extraDataHex) {
    ...
    if (!block) {
      const e = new Error(
        "Cannot create work template, check state before requesting work."
      );
      e.code = 501;
      throw e;
    }
    const shareCompact =
      this._miner.shareCompact ||
      Nimiq.BlockUtils.targetToCompact(block.target);
    const blockSerialized = block.serialize();
    return {
      data: Nimiq.BufferUtils.toHex(
        blockSerialized.subarray(0, block.header.serializedSize)
      ),
      suffix: Nimiq.BufferUtils.toHex(
        blockSerialized.subarray(
          block.header.serializedSize,
  ...
  async getBlockTemplate(addressStr, extraDataHex) {
    ...
    const shareCompact =
      this._miner.shareCompact ||
      Nimiq.BlockUtils.targetToCompact(block.target);
    const merklePath = await Nimiq.MerklePath.compute(
      block.body.getMerkleLeafs(),
      block.minerAddr
    );
    const merkleHashes = merklePath.nodes
      .map((mpn) => mpn.hash.toHex())
      .slice(1);
    return {
      header: {
      ...
      body: {
        ...
        transactions: block.body.transactions.map((tx) =>
          Nimiq.BufferUtils.toHex(tx.serialize())
        ),
        merkleHashes: merkleHashes,
        prunedAccounts: block.body.prunedAccounts.map((acc) =>
          Nimiq.BufferUtils.toHex(acc.serialize())
        ),
      },
    };
  }
  ...
  async submitBlock(blockHex) {
    ...
    if (!block.header.bodyHash.equals(block.body.hash()))
      throw new Error(
        "Submitted invalid block: bodyHash and body.hash() mismatch"
      );
    return this._miner.onWorkerShare({
      hash: await block.header.pow(),
  ...
  async accounts() {
    ...
    return addresses.map((address, i) =>
      this._accountToObj(accounts[i], address)
    );
  }
  ...
  async getBalance(addrString, atBlock) {
    if (atBlock && atBlock !== "latest")
      throw new Error(`Cannot calculate balance at block ${atBlock}`);
    return (await this._client.getAccount(Nimiq.Address.fromString(addrString)))
      .balance;
  }
  ...
  async _getBlockByNumber(number) {
    if (typeof number === "string") {
      if (number.startsWith("latest-")) {
        number =
          (await this._client.getHeadHeight()) - parseInt(number.substring(7));
  ...
  _infoToPeerObj(peerInfo, addressInfo) {
    ...
    return {
      ...
      headHash:
        peerInfo && peerInfo.headHash ? peerInfo.headHash.toHex() : undefined,
      ...
      obj.transactions = includeTransactions
        ? await Promise.all(
            block.transactions.map((tx, i) =>
              this._transactionToObj(tx, block, i)
            )
          )
  ...
  async _transactionToObj(tx, block, i) {
    return {
      ...
      timestamp: block ? block.timestamp : undefined,
      confirmations: block
        ? (await this._client.getHeadHeight()) - block.height + 1
        : 0,
      transactionIndex: i,
    ...
    req
      ...
      .on("end", async () => {
        ...
        for (const msg of body) {
          ...
          try {
            const methodRes = await this._methods
              .get(msg.method)
              .apply(
                null,
                msg.params instanceof Array ? msg.params : [msg.params]
              );
